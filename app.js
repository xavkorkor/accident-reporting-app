...
function renderDiagram(d){ const c = byId('diagramCanvas'), ctx = c.getContext('2d'); ctx.clearRect(0,0,c.width,c.height); ctx.fillStyle = '#e9ecef'; ctx.fillRect(0,0,c.width,c.height); ctx.fillStyle = '#555'; ctx.fillRect(270,60,220,640); ctx.strokeStyle = 'white'; ctx.setLineDash([24,24]); ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(380,70); ctx.lineTo(380,690); ctx.stroke(); ctx.setLineDash([]); ctx.fillStyle = '#111'; ctx.font = 'bold 24px Arial'; ctx.fillText('Traffic Flow ↑',40,50); const front = d.front_vehicle || 'A', rear = d.rear_vehicle || 'B'; drawCar(ctx,front,330,170,'FRONT'); drawCar(ctx,rear,330,430,'REAR'); ctx.fillStyle = '#d93025'; ctx.font = 'bold 70px Arial'; ctx.fillText('*',365,390); }
