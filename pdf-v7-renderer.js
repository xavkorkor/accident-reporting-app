...
(function () {
  ...
  function drawCar(ctx, label, x, y, tag) {
    ctx.save(); ctx.fillStyle = '#fff'; ctx.strokeStyle = '#111'; ctx.lineWidth = 4.5;
    roundRect(ctx, x, y, 52, 78, 9); ctx.fill(); ctx.stroke();
    ctx.lineWidth = 3.5; ctx.strokeRect(x + 11, y + 9, 30, 11); ctx.strokeRect(x + 11, y + 58, 30, 11);
    ctx.fillStyle = '#111'; ctx.font = 'bold 25px Arial'; ctx.fillText(label, x + 18, y + 47);
    if (tag) { ctx.font = 'bold 8px Arial'; ctx.fillText(tag, x + 15, y + 56); }
    ctx.restore();
  }

  function drawVan(ctx, label, x, y) {
    ctx.save(); ctx.fillStyle = '#fff'; ctx.strokeStyle = '#111'; ctx.lineWidth = 4.5;
    ctx.strokeRect(x + 2, y, 48, 82); ctx.lineWidth = 3.5; ctx.strokeRect(x + 10, y + 10, 32, 18); ctx.strokeRect(x + 10, y + 37, 32, 32);
    ctx.fillStyle = '#111'; ctx.font = 'bold 23px Arial'; ctx.fillText(label, x + 18, y + 54);
    ctx.font = 'bold 8px Arial'; ctx.fillText('VAN', x + 17, y + 93); ctx.restore();
  }

  function drawTruck(ctx, label, x, y) {
    ctx.save(); ctx.fillStyle = '#fff'; ctx.strokeStyle = '#111'; ctx.lineWidth = 4.5;
    ctx.strokeRect(x, y, 58, 84); ctx.lineWidth = 3.5; ctx.strokeRect(x + 9, y + 8, 40, 22); ctx.strokeRect(x + 9, y + 36, 40, 40);
    ctx.fillStyle = '#111'; ctx.font = 'bold 23px Arial'; ctx.fillText(label, x + 21, y + 58);
    ctx.font = 'bold 8px Arial'; ctx.fillText('TRUCK', x + 13, y + 95); ctx.restore();
  }

  function drawBus(ctx, label, x, y) {
    ctx.save(); ctx.fillStyle = '#fff'; ctx.strokeStyle = '#111'; ctx.lineWidth = 4.5;
    ctx.strokeRect(x, y, 58, 88); ctx.lineWidth = 3;
    for (let i = 0; i < 4; i++) { ctx.strokeRect(x + 7, y + 10 + i * 16, 11, 8); ctx.strokeRect(x + 40, y + 10 + i * 16, 11, 8); }
    ctx.fillStyle = '#111'; ctx.font = 'bold 23px Arial'; ctx.fillText(label, x + 22, y + 53);
    ctx.font = 'bold 8px Arial'; ctx.fillText('BUS', x + 21, y + 99); ctx.restore();
  }

  function drawMotorcycle(ctx, label, x, y) {
    ctx.save(); ctx.strokeStyle = '#111'; ctx.fillStyle = '#111';
    ctx.lineWidth = 5.5; ctx.beginPath(); ctx.ellipse(x + 26, y + 12, 11, 11, 0, 0, Math.PI * 2); ctx.stroke();
    ctx.beginPath(); ctx.ellipse(x + 26, y + 66, 11, 11, 0, 0, Math.PI * 2); ctx.stroke();
    ctx.lineWidth = 7.5; ctx.beginPath(); ctx.moveTo(x + 26, y + 24); ctx.lineTo(x + 26, y + 54); ctx.stroke();
    ctx.font = 'bold 21px Arial'; ctx.fillText(label, x + 18, y + 46);
    ctx.font = 'bold 8px Arial'; ctx.fillText('BIKE', x + 14, y + 88); ctx.restore();
  }
  ...
  function drawLegend(ctx, d) {
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.96)'; ctx.strokeStyle = '#111'; ctx.lineWidth = 2.6;
    ...
    rows.forEach((r, i) => {
      ...
      ctx.fillStyle = '#111'; ctx.font = 'bold 12px Arial'; ctx.fillText(r[0], x + 36, yy - 3);
      ctx.font = 'bold 10px Arial'; ctx.fillText((r[1] || '') + (r[2] ? ' ' + r[2] : ''), x + 58, yy - 3);
    });
    const iy = y + 38 + rows.length * 32;
    ctx.font = 'bold 19px Arial'; ctx.fillText('X', x + 14, iy - 2);
    ctx.font = 'bold 10px Arial'; ctx.fillText('Point of Impact', x + 36, iy - 4);
    ctx.restore();
  }
  ...
  window.renderDiagram = function renderDiagram(d) {
    ...
    ctx.strokeStyle = '#555'; ctx.lineWidth = 4; ctx.setLineDash([16, 16]);
    ...
    ctx.fillStyle = '#111'; ctx.font = 'bold 34px Arial'; ctx.fillText('X', x + 13, 168);
    ctx.font = 'bold 44px Arial'; ctx.fillText('↑', left + roadW / 2 - 13, 296);
